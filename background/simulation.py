...
def make_animal(spec: dict[str, Any], name: str, seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "confidence": rng.uniform(0.34, 0.72),
...
class AquariumSimulation:
    ...
    def _normalize_state(self) -> None:
        ...
        for animal in self.state.get("animals", []):
            ...
            animal.setdefault("confidence", 0.55)
            ...
            animal.setdefault("behavior_notes", [])
    ...
    def _update_behavior_memory(self, animal: dict[str, Any], spec: dict[str, Any], group: int, welfare_risk: dict[str, Any], resting: bool, hours: float) -> list[str]:
        aquarium = self.state["aquarium"]
        water = self.state["water"]
        equipment = self.state.get("equipment", {})
        filter_state = equipment.get("filter", {})
        flow = clamp(float(filter_state.get("effective_flow", filter_state.get("flow", 0.55))), 0.0, 1.0)
        hiding = clamp(float(aquarium.get("hiding_cover", 0.35)) + float(aquarium.get("plant_cover", 0.0)) * 0.22, 0.0, 1.0)
        open_swimming = clamp(float(aquarium.get("open_swimming", 0.55)) - float(aquarium.get("flow_break", 0.0)) * 0.28, 0.0, 1.0)
        stability = float(self.state.get("stability", {}).get("stability_score", 0.75))
        minimum = max(1, int(spec.get("minimum_group", 1)))
        preferred = max(minimum, int(spec.get("preferred_group", minimum)))
        group_ratio = clamp(group / max(1.0, float(preferred)), 0.0, 1.25)
        social = str(spec.get("social", ""))
        is_schooling = any(word in social for word in ("school", "shoal", "colony", "group"))
        activity = clamp(float(spec.get("activity", 0.5)), 0.0, 1.0)
        territoriality = clamp(float(spec.get("territoriality", 0.0)), 0.0, 1.0)
        stress = float(animal.get("acute_stress", 0.0))
        hunger = float(animal.get("hunger", 0.0))
        boldness = float(animal.get("boldness", 0.5))
        sociability = float(animal.get("sociability", 0.5))
        notes: list[str] = []

        flow_match = 1.0 - clamp(abs(flow - (0.68 if activity > 0.72 else 0.44 if spec.get("swim_zone") == "bottom" else 0.54)) / 0.62, 0.0, 1.0)
        habitat_match = clamp(hiding * 0.42 + open_swimming * (0.32 + activity * 0.22) + flow_match * 0.24, 0.0, 1.0)
        social_match = group_ratio if is_schooling else clamp(1.0 - max(0, group - preferred) * territoriality * 0.22, 0.0, 1.0)
        comfort_target = clamp(habitat_match * 0.42 + social_match * 0.26 + stability * 0.18 + (1.0 - stress) * 0.14, 0.0, 1.0)
        confidence_target = clamp(comfort_target * 0.58 + boldness * 0.28 + float(animal.get("health", 0.9)) * 0.14 - float(animal.get("fear_memory", 0.0)) * 0.24, 0.0, 1.0)
        animal["confidence"] = clamp(float(animal.get("confidence", 0.55)) + (confidence_target - float(animal.get("confidence", 0.55))) * min(1.0, hours * 0.045), 0.0, 1.0)

        feeding_penalty = 0.0
        reasons = " ".join(str(reason) for reason in welfare_risk.get("reasons", []))
        if "outcompeted" in reasons:
            feeding_penalty += 0.35
        if stress > 0.42 or float(animal.get("fear_memory", 0.0)) > 0.45:
            feeding_penalty += 0.22
        feeding_target = clamp(float(animal.get("feeding_rank", 0.6)) * 0.45 + float(animal.get("confidence", 0.5)) * 0.35 + float(animal.get("last_diet_match", 0.7)) * 0.20 - feeding_penalty, 0.0, 1.0)
        animal["feeding_opportunity"] = clamp(float(animal.get("feeding_opportunity", 0.65)) + (feeding_target - float(animal.get("feeding_opportunity", 0.65))) * min(1.0, hours * 0.07), 0.0, 1.0)
        ...
        if is_schooling:
            ...
            if group < minimum:
                ...
                notes.append("holding a tight school")
            elif animal["school_cohesion"] < 0.34:
        ...
        if intent_remaining <= 0.0:
            intent, intent_remaining, trigger, role = self._choose_behavior_intent(animal, spec, group, is_schooling, resting, flow, hiding, open_swimming, stress, hunger)
            animal["behavior_intent"] = intent
            animal["intent_hours_remaining"] = intent_remaining
            animal["behavior_trigger"] = trigger
            animal["school_role"] = role
            animal["decision_count"] = int(animal.get("decision_count", 0)) + 1
            history = list(animal.get("behavior_history", []))
            history.insert(0, f"{intent}: {trigger}")
            animal["behavior_history"] = history[:4]
        else:
            animal["intent_hours_remaining"] = intent_remaining
        ...
        if animal["territory_pressure"] > 0.52:
            notes.append("guarding a territory")

        rest_target = clamp(hiding * 0.38 + stability * 0.26 + (1.0 - stress) * 0.24 + (0.12 if resting else -0.08) - float(water.get("turbidity", 0.0)) * 0.12, 0.0, 1.0)
        animal["rest_quality"] = clamp(float(animal.get("rest_quality", 0.75)) + (rest_target - float(animal.get("rest_quality", 0.75))) * min(1.0, hours * (0.08 if resting else 0.025)), 0.0, 1.0)
        if resting and animal["rest_quality"] < 0.38:
            notes.append("sleep is disturbed")

        active_phase = 0.18 if resting else 0.72
        drive_target = clamp(active_phase * activity + hunger * 0.24 + float(animal.get("curiosity", 0.5)) * 0.18 + float(animal.get("confidence", 0.5)) * 0.12 - stress * 0.26, 0.0, 1.0)
        animal["activity_drive"] = clamp(float(animal.get("activity_drive", 0.5)) + (drive_target - float(animal.get("activity_drive", 0.5))) * min(1.0, hours * 0.08), 0.0, 1.0)

        if spec.get("swim_zone") == "bottom":
            animal["comfort_zone"] = "substrate and cover"
        elif spec.get("swim_zone") == "upper":
            animal["comfort_zone"] = "surface cover"
        elif is_schooling:
            animal["comfort_zone"] = "open middle water"
        else:
            animal["comfort_zone"] = "planted cover"
        animal["behavior_notes"] = notes[:4]
        return notes

    def _choose_behavior_intent(
        self,
        animal: dict[str, Any],
        spec: dict[str, Any],
        group: int,
        ...
        """A bounded, per-animal decision tree for short lived visible behavior.

        The choices are intentionally non-lethal. Water quality, welfare, and
        disease still drive health elsewhere; this makes individual fish look
        observant and responsive without turning harmless randomness into loss.
        """
        water = self.state["water"]
        social = str(spec.get("social", ""))
        zone = str(spec.get("swim_zone", "middle"))
        role = "follower" if is_schooling else "individual"
        candidates: list[tuple[str, float, str, float]] = []
        fear = clamp(float(animal.get("fear_memory", 0.0)), 0.0, 1.0)
        confidence = clamp(float(animal.get("confidence", 0.5)), 0.0, 1.0)
        curiosity = clamp(float(animal.get("curiosity", 0.5)), 0.0, 1.0)
        boldness = clamp(float(animal.get("boldness", 0.5)), 0.0, 1.0)
        health = clamp(float(animal.get("health", 0.9)), 0.0, 1.0)
        cohesion = clamp(float(animal.get("school_cohesion", 0.0)), 0.0, 1.0)
        ...
        # Immediate conditions win; they produce a short, readable response.
        if animal.get("disease") or health < 0.48:
            return "conserving energy", 0.16, "illness or low condition", role
        if oxygen_low or float(water.get("co2_mg_l", 4.0)) > 18.0:
            return "seeking surface", 0.08, "respiratory pressure", role
        if stress > 0.68 or fear > 0.72 or current_heavy:
            return "checking cover", 0.12, "stress, fear, or tiring flow", role
        if resting and float(animal.get("rest_quality", 0.75)) > 0.38:
            return "resting nearby", 0.24, "night rest", role

        if is_schooling:
            lead_drive = boldness * 0.44 + float(animal.get("feeding_rank", 0.6)) * 0.24 + confidence * 0.32
            if group < max(1, int(spec.get("minimum_group", 1))) or cohesion < 0.40:
                return "regrouping", 0.14, "school cohesion is low", "follower"
            candidates.extend([
                ("following", 3.5 + cohesion * 3.0 + (1.0 - fear), "comfortable group spacing", 0.18 + cohesion * 0.35),
                ("leading", max(0.08, lead_drive - 0.55) * 3.8, "confidence and feeding rank", 0.16 + lead_drive * 0.28),
                ("scouting", max(0.06, curiosity - 0.50) * 1.6, "curiosity at the school edge", 0.08 + curiosity * 0.14),
                ("feeding lane", max(0.05, hunger - 0.35) * 2.4 + feeding_opportunity * 0.35, "hunger and access to food", 0.10 + hunger * 0.16),
                ("current riding", max(0.05, flow - 0.34) * max(0.2, boldness) * 0.9, "comfortable flow", 0.12 + flow * 0.18),
            ])
        else:
            if hunger > 0.62:
                candidates.append(("foraging pass", 3.0 + hunger * 2.4, "hunger", 0.10 + hunger * 0.18))
            if cleanup_role:
                candidates.append(("grazing", 1.4 + float(cleanup_role.get("algae", 0.0)) + float(cleanup_role.get("biofilm", 0.0)), "cleanup diet and biofilm", 0.16 + curiosity * 0.22))
            if zone == "bottom":
                candidates.append(("sifting substrate", 1.3 + float(water.get("detritus", 0.0)) * 1.8, "bottom-dwelling routine", 0.14 + hunger * 0.16))
            if zone == "upper":
                candidates.append(("surface patrol", 1.0 + confidence * 0.8, "upper-water preference", 0.14 + curiosity * 0.18))
            if territory > 0.42:
                candidates.append(("territory patrol", territory * 2.6, "territory pressure", 0.16 + territory * 0.20))
            if cover_needed:
            ...
            candidates.extend([
                ...
                ("pausing nearby", 0.28 + max(0.0, 0.58 - confidence), "brief caution", 0.08 + fear * 0.14),
            ])

        total = sum(max(0.0, weight) for _intent, weight, _trigger, _duration in candidates)
        if total <= 0.0:
            return ("following", 0.20, "default social response", role) if is_schooling else ("exploring", 0.20, "default exploration", role)
        roll = self._rng(f"behavior_choice_{animal['id']}_{animal.get('decision_count', 0)}").random() * total
        selected = candidates[-1]
        for candidate in candidates:
            roll -= max(0.0, candidate[1])
            if roll <= 0.0:
                selected = candidate
                break
        intent, _weight, trigger, duration = selected
        if intent == "leading":
            role = "leader"
        elif intent == "scouting":
            role = "scout"
    ...
    def _update_animal(self, animal: dict[str, Any], groups: dict[str, int], welfare: dict[str, Any], consumed: float, hours: float) -> None:
        ...
        if animal.get("disease"):
            ...
            animal["routine"] = "hide"
        elif float(animal.get("territory_pressure", 0.0)) > 0.62 and animal["acute_stress"] < 0.46:
            animal["behavior"] = "guarding a favorite territory"
            ...
            if float(animal.get("rest_quality", 0.75)) < 0.42:
            ...
            if float(animal.get("feeding_opportunity", 0.65)) < 0.38:
                animal["behavior"] = "hungry but hanging back from food"
                animal["routine"] = "hang_back"
            else:
            ...
            animal["behavior"] = "staying close to cover"
            animal["routine"] = "hide"
        elif float(animal.get("activity_drive", 0.5)) > 0.78 and float(spec.get("activity", 0.5)) > 0.72:
            animal["behavior"] = "making active laps through open water"
            animal["routine"] = "pace"
        elif float(animal.get("feeding_opportunity", 0.65)) < 0.28 and animal["hunger"] > 0.52:
            animal["behavior"] = "watching food from behind stronger tankmates"
            animal["routine"] = "hang_back"
            ...
            animal["routine"] = "school"
        elif str(animal.get("behavior_intent", "")) == "conserving energy":
            animal["behavior"] = "conserving energy close to cover"
            animal["routine"] = "hide"
        elif str(animal.get("behavior_intent", "")) == "seeking surface":
            animal["behavior"] = "seeking easier breathing near the surface"
            animal["routine"] = "surface"
        elif str(animal.get("behavior_intent", "")) == "foraging pass":
            animal["behavior"] = "making a short foraging pass"
            animal["routine"] = "forage"
        elif str(animal.get("behavior_intent", "")) == "grazing":
            animal["behavior"] = "grazing algae and biofilm"
            animal["routine"] = "graze"
        elif str(animal.get("behavior_intent", "")) == "sifting substrate":
            animal["behavior"] = "sifting gently through the substrate"
            animal["routine"] = "sift"
        ...
        elif str(animal.get("behavior_intent", "")) == "current riding":
            animal["behavior"] = "riding the comfortable current"
            animal["routine"] = "current"
        elif str(animal.get("behavior_intent", "")) == "surface patrol":
            animal["behavior"] = "patrolling the upper water"
            animal["routine"] = "surface_patrol"
        elif str(animal.get("behavior_intent", "")) == "territory patrol":
            animal["behavior"] = "making a quiet territory circuit"
            animal["routine"] = "territory"
        ...
        if behavior_notes and animal["routine"] not in {"surface", "hide", "flee"}:
            animal["behavior"] = f"{animal['behavior']} ({behavior_notes[0]})"
